...
const { getAvailableSims } = require('../services/accountController/simService');
const { sendOTP } = require('../services/accountController/otpService');
const rateLimit = require('express-rate-limit');

const axios = require('axios');
const { sendMessageToDiscord } = require('../services/accountController/discordLogging');
...
const authUser = asyncHandler(async (req, res) => {
    ...
    if (lastOTPRequestTime && currentTime - lastOTPRequestTime < process.env.OTP_RESEND_WAIT_TIME){
        const timeLeft = process.env.OTP_RESEND_WAIT_TIME - (currentTime - lastOTPRequestTime);
    ...
    if (response.status === 200 || response.status === 202) {
        ...
        await redisClient.setEx(`OTP_${phone}`, process.env.OTP_VALID_TIME, String(otp));
        // Store last request timestamp (valid for 120s)
        await redisClient.setEx(`OTP_REQUEST_TIME_${phone}`, process.env.OTP_RESEND_WAIT_TIME, String(currentTime));
        // Increment SMS count in redis for current sim
        await redisClient.incr(creds.key);
        res.status(response.status).json({ success: 'true', message: 'We have sent a verification code to you via SMS' });
...
const verifyOTP = asyncHandler(async(req, res) => {
    ...
    //Logging to discord
    await sendMessageToDiscord(`User **${phone}** verified successfully ✅🤞🏻`, 5832630);
});
